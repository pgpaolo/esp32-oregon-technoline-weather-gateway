...
'''
    ...
    uv_helper = "const getUvRows=s=>(Array.isArray(s.oregon_sensors)?s.oregon_sensors:[]).filter(x=>x.t==='uv'&&Number(x.uv)>=0);const renderUvSensors=s=>{const rows=getUvRows(s);if(!rows.length)return '<div class=\\\"muted\\\">Nessun sensore UV ricevuto nella sessione.</div>';return rows.map(x=>'<div class=\\\"uvSensorItem\\\"><div class=\\\"uvSensorHead\\\"><b>'+x.m+' · ID '+x.id+'</b><span class=\\\"uvSensorValue\\\">'+Number(x.uv).toFixed(1)+'</span></div><small class=\\\"qmeta\\\">'+(Number(x.v)===2?'V2.1':'OSV3')+' · 0x'+x.c+' · CH'+x.ch+' · '+rssiBadge(x.rssi)+' · '+age(x.age)+'</small></div>').join('')};\n"
