...
<style>
:root{color-scheme:dark;--bg:#08111f;--panel:#0d1829;--panel2:#101d30;--panel3:#0a1525;--border:#26384e;--text:#e8eef8;--muted:#8fa7c5;--ok:#30d99a;--warn:#f0b24a;--bad:#ff7070;--blue:#83b7ff;--oregon:#3fd39b;--tech:#55aef6;--bme:#f0b24a;--lightning:#ffd166}
...
@media(max-width:1220px){.weatherGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.bmeGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.lightningGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.diagGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.fieldGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.rfControls{grid-template-columns:1fr 1fr}}
.thermoTabs{display:flex;gap:4px;margin-left:auto}.thermoTab{display:none;padding:3px 7px;font-size:.68rem}.thermoTab.show{display:inline-flex}.cardTitle:has(.thermoTabs){gap:6px;flex-wrap:wrap}
@media(max-width:760px){main{padding:9px}.title{font-size:1.08rem}.sub{font-size:.75rem}.headerActions{width:100%}.statusPill{padding:7px 9px}.mainTabs{position:sticky;top:0;z-index:5}.weatherGrid,.bmeGrid,.lightningGrid{grid-template-columns:1fr;padding:10px}.diagGrid,.resourceGrid,.resourceHeroGrid,.fieldGrid,.cfgGrid,.rfControls{grid-template-columns:1fr}.windCard .body{grid-template-columns:minmax(0,1fr) 100px}.windCompass{width:94px;height:94px}.spark{width:82px}.modeBox{align-items:flex-start}.modeBtn{padding:8px 10px}}
</style></head><body><main>
<div class="top"><div class="brand"><div class="title">Oregon + Technoline 433 Gateway</div><div class="sub">LILYGO T3 · SX1278 OOK 433.92 MHz · decoder Oregon OSV3 + Technoline WS230x</div></div><div class="headerActions"><span id="hdrRf" class="statusPill wait">RF --</span><span id="net" class="statusPill wait">Wi-Fi...</span><span id="hdrMqtt" class="statusPill wait">MQTT...</span><button id="displayBtn" class="modeBtn" onclick="toggleDisplay()">OLED --</button><button class="modeBtn dangerBtn" onclick="powerOffDevice()">⏻ SPEGNI</button><button class="modeBtn dangerBtn" onclick="restartDevice()">⟳ RIAVVIA</button></div></div>
...
<div class="row" id="thermoDerivedHeat"><div class="name">Heat index esterno</div><div class="value" id="hi">N/A</div></div>
<div class="row" id="thermoDerivedDew"><div class="name">Punto di rugiada</div><div class="value" id="dew">--</div></div>
...
</div><div class="cfgNote">Monitor da <code>/api/state</code>, senza scritture NVS.</div></div>
</section>
<section id="mainConfig" class="mainPage">
<div class="panel cfgPanel"><div class="panelHead">Configurazione dispositivo <span class="muted">NVS solo su modifica</span></div><div class="cfgTabs"><button id="tabNet" class="cfgTab active" onclick="showCfgTab('net')">RETE / IP</button><button id="tabThermo" class="cfgTab" onclick="showCfgTab('thermo')">OREGON</button><button id="tabMqtt" class="cfgTab" onclick="showCfgTab('mqtt')">MQTT / TLS</button><button id="tabDisplay" class="cfgTab" onclick="showCfgTab('display')">DISPLAY</button><button id="tabLightning" class="cfgTab" onclick="showCfgTab('lightning')">⚡ AS3935</button><button id="tabBackup" class="cfgTab" onclick="showCfgTab('backup')">BACKUP / RESTORE</button></div><div id="cfgNet" class="cfgPage active">
<div class="cfgGrid">
...
<div class="cfgNote">Le modifiche di rete richiedono il riavvio. mDNS: <code>http://hostname.local/</code>.</div>
</div>
<div id="cfgThermo" class="cfgPage">
<div class="cfgGrid">
...
<div class="cfgNote">Principale = meteo/MQTT legacy. Auto aggiunge i canali rilevati.</div>
</div>
...
<div class="cfgNote">TLS verificato usa la CA PEM. CH1-CH3 seguono OREGON. NVS solo su modifica.</div>
</div>
...
<div class="cfgNote"><b>Incluso:</b> hostname/IP, MQTT/TLS, campi MQTT, configurazione OLED, AS3935 e configurazione RF persistente. <b>Non incluso:</b> SSID/password Wi-Fi, che in questa versione restano nel firmware/config_private.h. Per sicurezza la password MQTT e' esclusa salvo selezione esplicita. L'import valida il file e riavvia il gateway.</div>
</div>
</div>
</section>
...
function displaySelectPages(v){document.querySelectorAll('[data-dpagebit]').forEach(x=>x.checked=!!v)}function bindDisplayFieldAutoPages(){const groups=[['data-denvbit',0],['data-dwindbit',1],['data-dtechbit',2],['data-dpressbit',3],['data-dstatusbit',4],['data-dasbit',5]];for(const [attr,bit] of groups){document.querySelectorAll('['+attr+']').forEach(x=>x.addEventListener('change',()=>{if(x.checked){const p=document.querySelector('[data-dpagebit=\"'+bit+'\"]');if(p)p.checked=true;}}));}}
async function loadDisplay(){try{const d=await (await fetch('/api/display/config',{cache:'no-store'})).json();E('dispOn').checked=!!d.on;E('dispInterval').value=d.page_interval_sec||7;E('dispContrast').value=d.contrast||255;dSet('data-dpagebit',d.page_mask==null?63:d.page_mask);dSet('data-denvbit',d.environment_fields==null?15:d.environment_fields);dSet('data-dwindbit',d.wind_rain_fields==null?15:d.wind_rain_fields);dSet('data-dtechbit',d.technoline_fields==null?31:d.technoline_fields);dSet('data-dpressbit',d.pressure_fields==null?15:d.pressure_fields);dSet('data-dstatusbit',d.status_fields==null?31:d.status_fields);dSet('data-dasbit',d.lightning_fields==null?31:d.lightning_fields);E('displaySummary').textContent=(d.on?'OLED ON':'OLED OFF')+' · pagina '+(Number(d.current_page)+1)+' · cambio '+d.page_interval_sec+' s · contrasto '+d.contrast+' · NVS '+(d.nvs_ok?'OK':'KO');}catch(e){E('displaySummary').textContent='errore lettura display'}}
async function saveDisplayConfig(){const pageMask=dGet('data-dpagebit');if(!pageMask){alert('Seleziona almeno una pagina OLED.');return}const q=new URLSearchParams();q.set('on',E('dispOn').checked?'1':'0');q.set('page_mask',String(pageMask));q.set('environment_fields',String(dGet('data-denvbit')));q.set('wind_rain_fields',String(dGet('data-dwindbit')));q.set('technoline_fields',String(dGet('data-dtechbit')));q.set('pressure_fields',String(dGet('data-dpressbit')));q.set('status_fields',String(dGet('data-dstatusbit')));q.set('lightning_fields',String(dGet('data-dasbit')));q.set('page_interval_sec',E('dispInterval').value);q.set('contrast',E('dispContrast').value);const r=await fetch('/api/display/config',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:q.toString(),cache:'no-store'});if(!r.ok){alert('DISPLAY: '+await r.text());return}const j=await r.json();displayOn=!!j.display_on;updateDisplayUi();await loadDisplay();}
...
async function refresh(){try{const s=await (await fetch('/api/state',{cache:'no-store'})).json(),w=s.weather,bme=s.bme280||{},p=s.packets,r=s.rf,a=s.fresh,ss=s.sensors,lc=s.lacrosse,lcr=s.lacrosse_rf,sess=s.session,b=s.burst,wp=s.wgr_probe||{},sys=s.system||{},th=s.oregon_thermo||{};
...
bindDisplayFieldAutoPages();loadNetwork();loadMqtt();refreshLightning();refresh();setInterval(refresh,2000);setInterval(refreshLightning,2000);setInterval(()=>{const editing=mainTab==='config'&&E('cfgMqtt')&&E('cfgMqtt').classList.contains('active');if(!editing)loadMqtt();},10000);
