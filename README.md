...
2. **Hardware** — ESP32 CPU, heap, flash, OTA space, RSSI, uptime and OLED state
3. **Configuration** — network and MQTT/TLS settings
4. **Diagnostics** — RF mode, gain/profile controls, acquisition state, RAW frames and burst diagnostics

The OLED can be placed in **power-save mode** from the Web UI. While disabled, display refreshes are suspended; RF reception, Wi-Fi, MQTT and the Web UI continue to operate normally. The OLED preference is persisted in NVS.
...
Edit `src/config_private.h` with your Wi-Fi, network and MQTT defaults.
...
The current power-saving feature is OLED shutdown/power-save from the Web UI.
